...
| **Audio not yet *clean*** | 🟠 | HDMI audio **works**, and the frame-rate **warble is FIXED** (`PIZERO-35/38/39`: streaming per-active-line delivery + SPSC/deepened ring — HW-confirmed steady tone). Residual gap vs desktop xroar is now **source-side** (`PIZERO-41`: ~1 semitone sharp + buzzier) — proven *not* HDMI (standard-timing + ACR sweeps had no effect; this sink ignores ACR). Likely a config/resampler difference; software-fixable. PWM sidecar remains a fallback. |
...
| **HDMI TV compatibility** | 🟠 | The **~52 Hz** off-spec refresh (24 MHz pixel = 240 MHz sysclk ÷ 10, forced by PIO-USB) is fine on tested monitors but a consumer-QA exposure — some sets may reject a non-60 Hz signal. NOT a render-speed limit (core 0 has ~4 ms/frame slack); it's the TMDS timing. Standard 640×480p60 needs a 25.175 MHz pixel → 251.75 MHz sysclk, which conflicts with PIO-USB's 240 MHz. `PIZERO-44`: investigate 60 Hz paths (PIO-USB at ~252 MHz to keep both; reduced-blanking 60 Hz at 24 MHz; or USB-less standard timing). 60 Hz would also fix game speed (currently ~13 % slow). |
...
| **USB-host stability (PIZERO-11b)** | 🟠 | Recurring system freezes (suspected USB-host path). **Mitigated:** `PIZERO-33` hardware watchdog is DEPLOYED + HW-validated — a wedged board now **auto-reboots in ~3 s** (was a dead brick) and records the stuck phase in scratch regs, so the next real freeze prints where core 0 hung. Root-cause still pending a real-world freeze capture (optional persistent-log enhancement to catch it without a serial monitor). |
